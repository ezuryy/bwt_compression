...
Главное свойство преобразования в том, что оно собирает вместе символы, соответствующие похожим контекстам. 
Чем больше стабильных контекстов в блоке данных, тем лучше будет сжиматься полученный в результате преобразования 
блок. Практика показывает, что в результате преобразования обычных текстов более половины из всех символов следует за такими же.  
...
алгоритма `O(NlogN)` (из-за сортировки), пространственная `O(N ^ 2)`  
**Недостатки:** пространственная сложность

#### Улучшенный прямой алгоритм преобразования Барроуза — Уилера

Идея состоит в использовании суффиксного дерева. Дописав в конец текста $, построим суффиксное дерево. Последовательно рассмотрим все внутренние
вершины и отсортируем детей каждой из них в алфавитном порядке. Вспомним, что в каждом листе
хранится номер конкретного суффикса. Двигаясь сверху вниз, составим последовательность из чисел 
во всех листьях. Теперь просматриваем эти числа и для каждого числа `i` пишем букву `s[i − 1]` (рис. 5).
Полученная последовательность букв будет результатом преобразования.
Сразу возникает логичный вопрос: как обойти все листья за линейное время? Для этого можно при менить обход в глубину.

Рассмотрим суффиксное дерево для строки `racaa$`. После добавления всех суффиксов ребра сортируются в алфавитном порядке. 
В листьях мы читаем порядок стартов суффиксов.  
...
Из каждого числа вычитаем единицу и получаем результат из
соответствующих букв текста: `s3`, `s1`, `s4`, `s2`, `s6`, `s5` = `craa$a`.  

![naive_bwt](report_pictures/suffix_tree.png)

**Достоинства:** временная сложность данного алгоритма `O(N)` (алгоритмом Укконена дерево строится за `O(N)`, 
обход в глубину `O(N)`), пространственная `O(N)`, т.к. мощность алфавита - константа, хоть и не маленькая.
**Недостатки:** использование символа `$`, наибольшего для нашего алфавита, делает невозможным использование этого 
символа в исходном тексте (потому что обратное преобразование не будет однозначным). Но можно реализовать суффиксное дерево без этого символа.
...
Чтобы заполнить промежуточную матрицу, нужно пройтись по входному слову за `N` и записать справа новый столбец. 
Далее строки матницы сортируется за `O(NlogN)`. Таких операций нужно сделать `N`, чтобы воспроизвести таблицу циклических сдвигов. Отсюда
временная сложность `O(N^3*logN)`.
...
Чтобы реализовать программно соответствие между элементами объекта `first` и элементами объекта `last`(п.2), нужно:  
1. Каждому символу входной строки поставить в соответствие индекс первого вхождения этого символа в отсортированной 
строке `first_positions`:
```
   p -> 9
   д -> 7
   а -> 0
   ...
   б -> 5
```
2. Составить вектор индексов `T`, который ставит в соответствие каждому индексу символа из `last` индекс 
именно этой буквы в отсортированном массиве. Т.к. нам известны индексы первых вхождений `first_positions`, то нужно лишь 
пройтись по символам `last`, находить текущий символ в `first_positions`, затем увеличивать значение этого индекса в `first_positions` на 1.  
...
Рассмотрим возможные алгоритмы сжатия:
...
Достоинства и недостатки метода Хаффмана/арифметического кодирования:
...
Рассмотрим возможные преобразования для улучшения качества сжатия:
...
Рассмотрим преобразования, являющиеся альтернативной BWT:
1. **RLE** Кодирование длин серий (англ. Run-length encoding, RLE) или **Кодирование повторов** - простой алгоритм сжатия данных, 
который оперирует сериями данных, то есть последовательностями, в которых один и тот же символ встречается несколько раз 
подряд. При кодировании строка одинаковых символов, составляющих серию, заменяется строкой, которая содержит сам 
повторяющийся символ и количество его повторов.
   Хорошо справится с последовательностями вида `WWWWWWWWWWWWBWWWWWWWWWWWWBBBWWWWWWWWWWWWWWWWWWWWWWWWBWWWWWWWWWWWWW
W->12W1B12W3B24W1B14W`, однако, в случае, если строка состоит 
из большого количества неповторяющихся символов, её объем может вырасти `ABCABCABCABCDDEFFFFFFFF->1A1B1C1A1B1C1A1B1C1A1B1C2D1E8F`.
2. **MTF** (move-to-front) позволяет легко преобразовать данные, насыщенные длинными повторами разных символов в блок данных, самыми частыми
символами которого будут нули. Изначально каждое возможное значение байта записывается в список (алфавит), в ячейку с 
номером, равным значению байта, т.е. (0,1,2,3,…,255). В процессе обработки данных этот список изменяется.
   По мере поступления очередного символа на выход подается номер элемента, содержащего его значение.
   После чего этот символ перемещается в начало списка, смещая остальные элементы вправо.
   Применив MTF-преобразование (предположим, начальный алфавит выглядит как "abcd") к строке `bbbbbcccccdddddaaaaa` получим
   `10000200003000030000`.
3. **PPM** (prediction by partial matching). Модель PPM использует контекст — множество символов в несжатом потоке, 
предшествующих данному, чтобы предсказывать значение символа на основе статистических данных. Сама модель PPM лишь
предсказывает значение символа, непосредственное сжатие осуществляется алгоритмами энтропийного кодирования, 
как например, алгоритм Хаффмана, арифметическое кодирование. Большое значение для алгоритма PPM имеет проблема обработки новых 
символов, ещё не встречавшихся во входном потоке. Это проблема носит название проблема нулевой частоты. Также требует 
достаточно больших затрат по памяти.
4. **Алгоритмы сжатия семейства LZ** являются методами словарного сжатия данных. Разработаны из предположения, что в данных часто встречаются повторяющиеся одинаковые 
подстроки. В настоящее время используются в файлах формата TIFF, PDF, GIF.

RLE и MTF имеет смысл применять, когда в данных много серий из одинаковых символов. Однако если таких последовательностей 
в исходных данных мало, то объем данных может сильно увеличиться, и преобразование само по себе не даст выгоды при сжатии. Поэтому их 
имеет смысл применять совместно с другим алгоритмом, например, с BWT. 

Современные реализации PPM являются одними из лучших среди алгоритмов, используемых для сжатия без потерь для текстов на 
естественном языке. При сжатии текста использование PPM предподчтительнее использования BWT.

Алгоритмы сжатия семейства LZ* используются в основном для сжатия изображений, но также эффективны при работе с текстом. 
Сравнивать их с одним BWT нет смысла, но если говорить о BWT + кодирование Хаффмана/арифметическое кодирование для работы с 
текстом, то LZ* более эффективны.
